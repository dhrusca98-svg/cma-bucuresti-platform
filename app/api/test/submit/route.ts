  ...
  try {
    ...
    if (
      ...
      return Response.json(
        {
          error:
            "Datele testului nu sunt valide.",
        },
        { status: 400 }
      );
    }
     ...
     *
     * Nu salvăm nimic,
     * dar verificăm răspunsul
     * pe server astfel încât
     * nici măcar preview-ul
     * să nu aibă nevoie de
     * correct_answer în browser.
     */
    if (
      context.isAdmin
    ) {
    ...
    return Response.json(
      {
        error:
          "Acțiunea solicitată nu este validă.",
      },
      { status: 400 }
    );
  } catch (error) {
