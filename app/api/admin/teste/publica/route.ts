...
export async function POST(
  ...
  try {
    ...
    const availabilityDays =
      Number(body.availabilityDays);

    if (!testId) {
      return Response.json(
        {
          error:
            "Lipsește testul selectat.",
        },
        { status: 400 }
      );
    }
    ...
    if (
      ...
      return Response.json(
        {
          error:
            "Durata testului trebuie să fie între 1 și 30 de zile.",
        },
        { status: 400 }
      );
    }
    ...
    const {
      ...
      .from("tests")
      .select(
        "id, title, is_active, available_until"
      )
      .eq("id", testId)
      .maybeSingle();
    ...
    return Response.json({
      ...
      message:
        `„${selectedTest.title}” a fost publicat pentru ${availabilityDays} ${
          availabilityDays === 1
            ? "zi"
            : "zile"
        }.`,
  ...
  try {
    ...
    if (
      ...
      return Response.json({
        success: true,
        message:
          `„${selectedTest.title}” este deja inactiv.`,
      });
    }
    ...
    return Response.json({
      success: true,
      message:
        `„${selectedTest.title}” a fost dezactivat. Rezultatele și răspunsurile existente au fost păstrate.`,
    });
  } catch (error) {
    console.error(
      "Eroare la dezactivarea testului:",
      error
    );
